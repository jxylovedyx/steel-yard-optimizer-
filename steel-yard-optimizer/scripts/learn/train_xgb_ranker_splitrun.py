...
def split_by_run(df: pd.DataFrame, groups: List[int], test_size: float, seed: int) -> Tuple[pd.DataFrame, List[int], pd.DataFrame, List[int]]:
    if "group_id" not in df.columns:
        raise ValueError("dataset missing group_id column")

    
    run_of_group: List[str] = []
    start = 0
    for gsz in groups:
        gid = str(df.iloc[start]["group_id"])
        run_id = gid.split(":", 1)[0]
        run_of_group.append(run_id)
        start += gsz

    uniq_runs = sorted(set(run_of_group))
    tr_runs, va_runs = train_test_split(uniq_runs, test_size=test_size, random_state=seed, shuffle=True)
    tr_set = set(tr_runs)
    va_set = set(va_runs)
...
def main() -> None:
    ...
    ap.add_argument("--out", default="models/xgb_ranker_splitrun.json")
    ...
    for c in FEATURES + ["label", "group_id"]:
    ...
    df_tr, g_tr, df_va, g_va = split_by_run(df, groups, args.test_size, args.seed)
